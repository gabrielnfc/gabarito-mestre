...
Fix wave final: b289fc1 (C1, M7, `.hashes-anteriores`, cenários 8/8b) · 3edc7e1 (I1, I2/F2-R7, I4/F2-R8, M1/F2-R10, M2, M5) · ad55d4b (C1 gap sem flags, cenários 9/9b — ruling F2-R11: rodada 2 escopada porque o modo padrão do instalador ainda violava F2-R9). Re-reviews confirmaram cada item por mutação e reprodução real do upgrade 1.0.1→1.1.0. Tree final: gates 236 mjs + 27 ts · instalador 70 · hooks 148 · validate ok · doctor sem autodetecção.

## CORTE DA SESSÃO (com motivo)
2026-09-24 — Fase 2 completa; PR #2 aberta; janela de contexto da sessão em ~65%, handoff por decisão do usuário. **Retomar:** após o merge da PR #2, fase 3 (hooks, T12–T16) em worktree `enabler/GM-3-hooks` a partir de `main`; ler `docs/superpowers/plans/2026-09-24-workflow-true-1.1.0.md` (contrato + Emendas, inclusive as da fase 2 para a fase 3: `GABARITO_PID_RAIZ=$PPID`, sempre `--plugin-root`) e o plano `…-fase-3-hooks.md`; briefs via extrator por cabeçalho `## T<N> —`; um implementador por vez; reviews com mutações executadas; final em modelo mais capaz; commits com trailers `Co-Authored-By` + `Claude-Session`. Memória do projeto tem o ponteiro.
